...
export async function authorizeActor(id, permission, transaction) {
  ...
  assert(!permission || publicUser(user).permisos.includes(permission), 403, 'FORBIDDEN', 'No tenés permiso para esta acción.');
  return user;
}
export async function listUsers(actorId, query) {
  return verified(async transaction => {
    ...
    const where = {};
    if (query.q) where[Op.or] = ['nombre', 'apellido', 'email'].map(field => ({ [field]: { [Op.like]: `%${query.q}%` } }));
    if (query.estado) where.estado = query.estado;
    const { count, rows } = await models.usuarios.findAndCountAll({ where, include: [{ model: models.roles, as: 'roles', through: { attributes: [] } }], distinct: true, order: [['fecha_registro', 'DESC']], offset: (query.page - 1) * query.limit, limit: query.limit, transaction });
    return { total: count, items: rows.map(u => { const { telefono, fecha_nacimiento, consentimiento_ia, ...publicFields } = publicUser(u); return publicFields; }) };
  });
}
export async function listRoles(actorId) {
  return verified(async transaction => {
    await authorizeActor(actorId, 'roles:manage', transaction);
    return (await models.roles.findAll({ order: [['nombre', 'ASC']], attributes: { exclude: ['dvh'] }, transaction })).map(r => r.toJSON());
  });
}
export async function saveRole(actorId, id, data) {
  return atomic(async transaction => {
    await authorizeActor(actorId, 'roles:manage', transaction);
    const existing = await models.roles.findOne({ where: { nombre: data.nombre, ...(id ? { id: { [Op.ne]: id } } : {}) }, transaction });
    ...
    if (id) {
      ...
      assert(!role.sistema, 409, 'SYSTEM_ROLE', 'Los roles del sistema están protegidos. Creá uno personalizado.');
      await role.update(data, { transaction });
    } else role = await models.roles.create({ ...data, fecha_creacion: new Date() }, { transaction });
    await audit(transaction, actorId, id ? 'CU035_MODIFICAR_ROL' : 'CU033_CREAR_ROL', 'administracion');
    const { dvh, ...result } = role.toJSON(); return result;
  });
}
export async function deleteRole(actorId, id) {
  return atomic(async transaction => {
    ...
    assert(!await models.usuario_roles.count({ where: { rol_id: id }, transaction }), 409, 'ROLE_ASSIGNED', 'Quitá las asignaciones antes de eliminar el rol.');
    await role.destroy({ transaction }); await audit(transaction, actorId, 'CU036_ELIMINAR_ROL', 'administracion');
  });
}
export async function assignRoles(actorId, targetId, roleIds) {
  return atomic(async transaction => {
    ...
    if (!roles.some(r => r.sistema && r.nombre === 'Administrador')) await assertAdminRemains(targetId, transaction);
    await models.usuario_roles.destroy({ where: { usuario_id: targetId }, transaction });
    await models.usuario_roles.bulkCreate(roles.map(r => ({ usuario_id: targetId, rol_id: r.id, fecha_asignacion: new Date() })), { transaction });
...
export async function changeStatus(actorId, targetId, estado) {
  return atomic(async transaction => {
    ...
    await models.sesiones.destroy({ where: { usuario_id: targetId }, transaction });
    await audit(transaction, actorId, estado === 'activo' ? 'CU038_ACTIVAR_USUARIO' : 'CU037_BLOQUEAR_USUARIO', 'administracion');
...
export async function removeUser(actorId, targetId) {
  return atomic(async transaction => {
    await authorizeActor(actorId, 'users:manage', transaction);
    assert(await models.usuarios.findByPk(targetId, { transaction }), 404, 'NOT_FOUND', 'No se encontró el usuario.');
...
export async function listAudit(actorId, query) {
  return verified(async transaction => {
    ...
    const { count, rows } = await models.bitacora.findAndCountAll({ where, order: [['fecha_hora', 'DESC']], attributes: { exclude: ['dvh'] }, offset: (query.page - 1) * query.limit, limit: query.limit, transaction });
