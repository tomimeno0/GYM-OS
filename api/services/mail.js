...
export async function sendRecovery(email, token) {
  ...
  const message = {
    from: process.env.MAIL_FROM || 'GYM-OS <no-reply@gym-os.local>',
    to: email,
    subject: 'Restablecé tu contraseña de GYM-OS',
  ...
  if (!process.env.SMTP_HOST)
    throw new AppError(
      503,
      'MAIL_UNAVAILABLE',
      'El correo no está configurado. Contactá al administrador.',
    );
  ...
  try {
    ...
    throw new AppError(503, 'MAIL_UNAVAILABLE', 'No se pudo enviar el correo. Intentá nuevamente.');
  }
}
