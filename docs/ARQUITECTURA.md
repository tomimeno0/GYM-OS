...
IA es un adaptador con límites de tiempo, salida estructurada, validación de esquema y contexto mínimo sin identidad. Perfil físico sólo se transmite tras consentimiento dentro del producto si se configura un proveedor remoto. Opción local para ejecutar sin compartir datos. El modelo no ejecuta herramientas ni consultas SQL. Errores no producen dietas/rutinas falsas.
...
Node 24, MySQL 8.4 aislado en Docker; Compose para reproducción. Desarrollo local usa puerto API 3000, Vite 5173, MySQL 3307 (no interfiere con instalaciones existentes), correo local de desarrollo y proveedor IA configurable. Entorno de pruebas con DB separada `gym_os_test`. En esta computadora se utiliza el contexto Docker `colima-gymos`, sin alterar el contexto predeterminado.
